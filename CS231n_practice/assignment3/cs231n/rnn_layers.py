...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    H = Wh.shape[0]
    prev_out, cache1 = affine_forward(prev_h, Wh, np.zeros(H)) # prev_h @ Wh 
    x_out, cache2 = affine_forward(x, Wx, b)# x @ Wx + b
    
    next_h = np.tanh(prev_out + x_out)
    
    cache = (next_h, cache1, cache2)
...
def rnn_step_backward(dnext_h, cache):
    ...
    next_h, cache1,cache2 = cache
    
    dtanh = np.multiply((1 - next_h**2), dnext_h)
    dprev_h, dWh, _ = affine_backward(dtanh, cache1)
    dx, dWx, db = affine_backward(dtanh, cache2)
...
def rnn_backward(dh, cache):
    ...
    N,T,H = dh.shape
    i = T - 1
    dx_temp, dprev_h, dWx, dWh, db = rnn_step_backward(dh[:,i,:], cache[0])
    D = dx_temp.shape[1]
    dx = np.zeros((N,T,D))
    dx[:,i,:] = dx_temp
    i -= 1
    ...
    for c in cache[1:]:
        dh_curr += dh[:,i,:]
        dx_temp, dh_curr, dWx_temp, dWh_temp, db_temp = rnn_step_backward(dh_curr, c)
        dWx += dWx_temp
        dWh += dWh_temp
        db += db_temp
        dx[:,i,:] = dx_temp
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    temp1, cache_1 = affine_forward(x, Wx, np.zeros(4*H))
    temp2, cache_2 = affine_forward(prev_h, Wh, b)
    a = temp1 + temp2
    ...
    cache = (cache_1, cache_2, ai, af, ao, ag, i, f, o, g, next_c, prev_c, prev_h)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    cache_1, cache_2, ai, af, ao, ag, i, f, o, g, next_c, prev_c, prev_h = cache
    
    d_o = np.tanh(next_c) * dnext_h
    ...
    d_a = np.concatenate((d_ai,d_af,d_ao,d_ag), axis=1)
    
    dx,dWx,_ = affine_backward(d_a, cache_1)
    dprev_h,dWh,db = affine_backward(d_a, cache_2)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    next_h = h0
    for t in range(T):
        next_h, prev_c, temp_cache = lstm_step_forward(x[:,t,:], next_h, prev_c, Wx, Wh, b)
        h[:,t,:] = next_h
        cache.append(temp_cache)
...
def lstm_backward(dh, cache):
    ...
    dx_temp, dprev_h, dnext_c, dWx, dWh, db = lstm_step_backward(dh[:,i,:], dnext_c, cache[0])
    D = dx_temp.shape[1]
    dx = np.zeros((N,T,D))
    ...
    for c in cache[1:]:
        dh_curr += dh[:,i,:]
        dx_temp, dh_curr, dnext_c, dWx_temp, dWh_temp, db_temp = lstm_step_backward(dh_curr, dnext_c, c)
        dWx += dWx_temp
        dWh += dWh_temp
        db += db_temp
        dx[:,i,:] = dx_temp
